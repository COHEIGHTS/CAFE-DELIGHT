<!DOCTYPE html>
<html lang="en" x-data="verifyEmailPage()" :class="{ 'dark': darkMode }" x-cloak class="scroll-smooth">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>Verify Email — Cafe Delight</title>
    ...
    <div class="mx-auto grid min-h-screen max-w-7xl items-center gap-0 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:px-8">
        ...
        <div class="relative hidden overflow-hidden rounded-l-[2.5rem] bg-gradient-to-br from-brand-500 via-brand-600 to-brand-800 p-12 text-white lg:flex lg:flex-col lg:justify-between lg:min-h-[600px]">
            ...
            <div class="relative">
                ...
                <div class="pointer-events-none absolute top-24 -left-2 text-5xl animate-float">✉️</div>
                <div class="pointer-events-none absolute bottom-2 right-12 text-5xl animate-float-slow">🔔</div>

                <h1 class="font-display text-4xl font-extrabold leading-tight xl:text-5xl">Verify<br>your email</h1>
                <p class="mt-5 max-w-md text-orange-50/90">Thanks for signing up! Before getting started, please verify your email address by clicking on the link we just emailed to you.</p>
                ...
                <div class="mt-8 flex items-center gap-1 text-gold">
                    ...
                    <span class="ml-2 text-sm font-semibold text-orange-50/90">Secure verification</span>
                </div>
            </div>

            <p class="text-sm text-orange-50/80">"Quick verification process — I was ordering my favorite shawarma in no time!" — Emma L.</p>
        </div>
        ...
        <div data-aos="fade-left" class="glass-strong rounded-[2.5rem] p-8 shadow-premium sm:p-10 lg:rounded-l-none lg:rounded-r-[2.5rem] lg:min-h-[600px] lg:flex lg:flex-col lg:justify-center">
            ...
            <div class="mx-auto w-full max-w-md">
                <h2 class="font-display text-3xl font-extrabold sm:text-4xl">Verify Email</h2>
                <p class="mt-2 text-ink/60 dark:text-orange-50/60">Check your inbox for the verification link.</p>
                ...
                @if (session('status') == 'verification-link-sent')
                    <div class="mt-6 flex items-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-sm font-semibold text-emerald-700 dark:text-emerald-400">
                        <i data-lucide="check-circle-2" class="h-5 w-5"></i> A new verification link has been sent to your email.
                    </div>
                @endif
                ...
                <div class="mt-8 space-y-4">
                    <form method="POST" action="{{ route('verification.send') }}" x-on:submit="submitting = true">
                        ...
                        <button type="submit" :disabled="submitting"
                            ...
                            <span x-show="!submitting" class="flex items-center gap-2">
                                Resend Verification Email <i data-lucide="send" class="h-5 w-5 transition-transform group-hover:translate-x-1"></i>
                            </span>
                            <span x-show="submitting" class="flex items-center gap-2">
                                <i data-lucide="loader-2" class="h-5 w-5 animate-spin"></i> Sending...
                            </span>
                        </button>
                    </form>
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="w-full rounded-xl border-2 border-ink/20 py-3.5 text-sm font-bold text-ink/70 transition hover:border-brand-500 hover:text-brand-600 dark:border-orange-50/20 dark:text-orange-50/70 dark:hover:border-brand-500 dark:hover:text-brand-500">
                            Log Out
                        </button>
                    </form>
                </div>
