<!DOCTYPE html>
<html lang="en" x-data="resetPasswordPage()" :class="{ 'dark': darkMode }" x-cloak class="scroll-smooth">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>Reset Password — Cafe Delight</title>
    ...
    <div class="mx-auto grid min-h-screen max-w-7xl items-center gap-0 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:px-8">
        ...
        <div class="relative hidden overflow-hidden rounded-l-[2.5rem] bg-gradient-to-br from-brand-500 via-brand-600 to-brand-800 p-12 text-white lg:flex lg:flex-col lg:justify-between lg:min-h-[600px]">
            ...
            <div class="relative">
                ...
                <h1 class="font-display text-4xl font-extrabold leading-tight xl:text-5xl">Set new<br>password</h1>
                <p class="mt-5 max-w-md text-orange-50/90">Create a strong new password to secure your account and continue enjoying your favorite meals.</p>
                ...
                <div class="mt-8 flex items-center gap-1 text-gold">
                    ...
                    <span class="ml-2 text-sm font-semibold text-orange-50/90">Secure password reset</span>
                </div>
            </div>

            <p class="text-sm text-orange-50/80">"Quick and secure — I was back to ordering in no time with a new password." — Mike R.</p>
        </div>
        ...
        <div data-aos="fade-left" class="glass-strong rounded-[2.5rem] p-8 shadow-premium sm:p-10 lg:rounded-l-none lg:rounded-r-[2.5rem] lg:min-h-[600px] lg:flex lg:flex-col lg:justify-center">
            ...
            <div class="mx-auto w-full max-w-md">
                <h2 class="font-display text-3xl font-extrabold sm:text-4xl">Reset Password</h2>
                <p class="mt-2 text-ink/60 dark:text-orange-50/60">Enter your new password below.</p>

                <form method="POST" action="{{ route('password.store') }}" class="mt-8 space-y-5" x-on:submit="submitting = true">
                    @csrf
                    ...
                    {{-- Password --}}
                    <div>
                        <label for="password" class="mb-1.5 block text-sm font-bold">New Password</label>
                        <div class="relative">
                    ...
                    {{-- Confirm Password --}}
                    <div>
                        <label for="password_confirmation" class="mb-1.5 block text-sm font-bold">Confirm New Password</label>
                        <div class="relative">
                    ...
                    <button type="submit" :disabled="submitting"
                        ...
                        <span x-show="!submitting" class="flex items-center gap-2">
                            Reset Password <i data-lucide="refresh-cw" class="h-5 w-5 transition-transform group-hover:rotate-180"></i>
                        </span>
                        <span x-show="submitting" class="flex items-center gap-2">
                            <i data-lucide="loader-2" class="h-5 w-5 animate-spin"></i> Resetting...
                        </span>
                    </button>
                </form>

                <p class="mt-6 text-center text-sm text-ink/60 dark:text-orange-50/60">
                    <a href="{{ route('login') }}" class="font-bold text-brand-600 underline-offset-4 transition hover:underline">Back to login</a>
                </p>
            </div>
        </div>
    </div>
