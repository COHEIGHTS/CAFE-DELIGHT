<!DOCTYPE html>
<html lang="en" x-data="confirmPasswordPage()" :class="{ 'dark': darkMode }" x-cloak class="scroll-smooth">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <title>Confirm Password — Cafe Delight</title>
    ...
    <div class="mx-auto grid min-h-screen max-w-7xl items-center gap-0 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:px-8">
        ...
        <div class="relative hidden overflow-hidden rounded-l-[2.5rem] bg-gradient-to-br from-brand-500 via-brand-600 to-brand-800 p-12 text-white lg:flex lg:flex-col lg:justify-between lg:min-h-[600px]">
            ...
            <div class="relative">
                ...
                <h1 class="font-display text-4xl font-extrabold leading-tight xl:text-5xl">Secure<br>your account</h1>
                <p class="mt-5 max-w-md text-orange-50/90">This is a secure area. Please confirm your password to continue accessing your account features.</p>
                ...
                <div class="mt-8 flex items-center gap-1 text-gold">
                    ...
                    <span class="ml-2 text-sm font-semibold text-orange-50/90">Enhanced security</span>
                </div>
            </div>

            <p class="text-sm text-orange-50/80">"Extra security gives me peace of mind when managing my orders." — Sarah M.</p>
        </div>
        ...
        <div data-aos="fade-left" class="glass-strong rounded-[2.5rem] p-8 shadow-premium sm:p-10 lg:rounded-l-none lg:rounded-r-[2.5rem] lg:min-h-[600px] lg:flex lg:flex-col lg:justify-center">
            ...
            <div class="mx-auto w-full max-w-md">
                <h2 class="font-display text-3xl font-extrabold sm:text-4xl">Confirm Password</h2>
                <p class="mt-2 text-ink/60 dark:text-orange-50/60">Please confirm your password to continue.</p>

                <form method="POST" action="{{ route('password.confirm') }}" class="mt-8 space-y-5" x-on:submit="submitting = true">
                    @csrf
                    ...
                    <button type="submit" :disabled="submitting"
                        ...
                        <span x-show="!submitting" class="flex items-center gap-2">
                            Confirm <i data-lucide="shield-check" class="h-5 w-5 transition-transform group-hover:translate-x-1"></i>
                        </span>
                        <span x-show="submitting" class="flex items-center gap-2">
                            <i data-lucide="loader-2" class="h-5 w-5 animate-spin"></i> Confirming...
                        </span>
                    </button>
                </form>

                <p class="mt-6 text-center text-sm text-ink/60 dark:text-orange-50/60">
                    <a href="{{ route('logout') }}" class="font-bold text-brand-600 underline-offset-4 transition hover:underline">Log out</a>
                </p>
            </div>
        </div>
    </div>
